...
export class DatabaseStorage implements IStorage {
  ...
  async addPropertyImages(images: InsertPropertyImage[]): Promise<PropertyImage[]> {
    ...
    return await db.insert(propertyImages).values(images).returning();
  }
  ...
  async getUnit(unitId: string): Promise<UnitWithImages | undefined> {
    ...
    return { ...unitData, images: unitData.images, propertyTitle: property?.title };
  }
  ...
  async addUnitImages(images: InsertUnitImage[]): Promise<UnitImage[]> {
    if (images.length === 0) return [];
    return await db.insert(unitImages).values(images).returning();
  }
  ...
  async getPropertyWithUnits(propertyId: string): Promise<PropertyWithUnitsAndImages | undefined> {
    return await db.query.properties.findFirst({
      ...
      with: {
        images: { orderBy: (images, { asc }) => [asc(images.displayOrder)] },
        units: {
          with: { images: { orderBy: (images, { asc }) => [asc(images.displayOrder)] } },
          orderBy: (units, { asc }) => [asc(units.unitNumber)],
        },
      },
    });
  ...
  async getAllPropertyViewCounts(): Promise<Array<{ propertyId: string; viewCount: number }>> {
    ...
    return result.map(r => ({ propertyId: r.propertyId, viewCount: Number(r.viewCount) }));
  }
  ...
  async createHeroImage(data: InsertHeroImage): Promise<HeroImage> {
    const [image] = await db.insert(heroImages).values(data).returning();
    return image;
  }
  ...
  async reorderHeroImages(imageIds: string[]): Promise<void> {
    for (let i = 0; i < imageIds.length; i++) {
      await db.update(heroImages).set({ displayOrder: i }).where(eq(heroImages.id, imageIds[i]));
    }
  }

  async createApartmentFinderSubmission(data: InsertApartmentFinderSubmission): Promise<ApartmentFinderSubmission> {
    const [submission] = await db.insert(apartmentFinderSubmissions).values(data).returning();
    return submission;
  }

  async getAllApartmentFinderSubmissions(): Promise<ApartmentFinderSubmission[]> {
    return await db.query.apartmentFinderSubmissions.findMany({
