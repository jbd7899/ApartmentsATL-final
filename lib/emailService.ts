...
export async function sendApartmentFinderNotification(submission: ApartmentFinderSubmission): Promise<void> {
  ...
  const htmlContent = `
    ...
    ${submission.amenities ? `<h3>Desired Amenities</h3><p>${submission.amenities}</p>` : ''}
    ${submission.additionalInfo ? `<h3>Additional Information</h3><p>${submission.additionalInfo}</p>` : ''}
